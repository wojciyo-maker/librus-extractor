...
router.get('/', (req, res) => {
  ...
  const users = db.prepare('SELECT id, username, label FROM secrets ORDER BY id').all();
...
router.post('/', (req, res) => {
  ...
  db.prepare('INSERT INTO secrets (id, username, password, label) VALUES (?, ?, ?, ?)').run(nextId, username, password, label || null);
  res.status(201).json({ id: nextId, username, label: label || null, is_active: false });
});
