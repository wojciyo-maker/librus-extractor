...
module.exports = class Inbox extends Resource {
  ...
  listReceivers(groupId = 0, isVirtualClasses = false) {
    ...
    // Fetch the page once to extract both class ID and CSRF token
    return this.api._request("GET", "wiadomosci/2/5").then((data) => {
      const html = data.html();

      // Extract CSRF token
      const csrfTokenMatch = html.match(/var csrfTokenValue = "([^"]+)"/);
      if (!csrfTokenMatch) {
        throw new Error("CSRF token not found in page");
      }
      const csrfToken = csrfTokenMatch[1];

      // Extract class ID (same logic as _getClassId)
      let classId = null;
      ...
      // Make requests for all recipient types using the session-aware caller
        const FormData = require("form-data");
        const caller = this.api.caller;
