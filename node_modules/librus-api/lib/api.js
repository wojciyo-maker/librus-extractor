...
class Librus {
  ...
  async authorize(login, pass) {
    ...
    try {
      await caller.get("https://synergia.librus.pl/loguj/portalRodzina", {
        headers: {
          Referer: "https://portal.librus.pl/"
        }
      });
      
      await caller.get("https://api.librus.pl/OAuth/Authorization?client_id=46&response_type=code&scope=mydata");
      
      await caller.postForm(
      ...
      await caller.postForm("https://api.librus.pl/OAuth/Authorization/2FA?client_id=46", {
        action: "requiredActions",
        skip: "true",
      });
      
      await caller.get("https://api.librus.pl/OAuth/Authorization/Grant?client_id=46");
      
      return await this.cookie.getCookies(config.page_url);
    } catch (error) {
      console.error(error);
      throw error;
    }
